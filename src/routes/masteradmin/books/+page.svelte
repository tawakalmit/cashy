<script lang="ts">
	...
	onMount(async () => {
		...
		const res = await fetch('/masteradmin/api/books', {
			method: 'POST',
...
<div class="w-full min-h-[100dvh] bg-[#ecf0f1]">
	<div class="w-full py-4 px-6 bg-[#2c3e50] flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<h1 class="text-white titillium-web-bold text-lg">Data Buku</h1>
		</div>
	</div>
			...
			<table class="w-full text-sm text-left">
				...
				<tbody>
					...
					<tr class="border-b border-gray-100 hover:bg-gray-50">
						...
						<td class="px-4 py-3">{book.profiles?.name || book.profiles?.email || '-'}</td>
						<td class="px-4 py-3">{Array.isArray(book.members) ? book.members.length : 0}</td>
						<td class="px-4 py-3">{new Date(book.created_at).toLocaleDateString('id-ID')}</td>
					</tr>
					{/each}
				</tbody>
			</table>
